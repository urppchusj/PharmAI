...
Preprocess the data extracted from the pharmacy database before input into the machine learning model
'''
...
class preprocessor():
	...
	def __init__(self, source_file, definitions_file, restrict_data, logging_level=logging.DEBUG):
		...
		# Settings
		self.data_save_path = os.path.join(os.getcwd(), 'preprocessed_data', 'whole_profile')
		self.profile_col_names = ['enc', 'date_beg', 'time_beg', 'date_end', 'time_end', 'medinb', 'dose', 'freq', 'genenb', 'date_begenc', 'date_endenc', 'time_endenc', 'depa', 'protoc', 'tram']
		self.profile_dtypes = {'enc':np.int32, 'date_beg':str, 'time_beg':str, 'date_end':str, 'time_end':str, 'medinb':str, 'dose':np.float32, 'freq':np.int32, 'genenb':str, 'date_begenc':str, 'date_endenc':str, 'time_endenc':str, 'depa':str, 'protoc':str, 'tram':str}
		self.definitions_col_names = ['medinb', 'mediname', 'genenb', 'genename', 'classnb', 'classname']
		self.definitions_dtypes = {'medinb':np.int32, 'mediname':str, 'genenb':str, 'genename':str, 'classnb':str, 'classename':str}

		# Congigure logger
		print('Configuring logger...')
		self.logging_path = os.path.join(os.getcwd(), 'logs', 'preprocessing', 'whole_profile')
		...
		self.raw_profile_data = pd.read_csv(source_file, sep=';', names=self.profile_col_names, index_col=None, dtype=self.profile_dtypes)
		classes_data = pd.read_csv(definitions_file, sep=';', names=self.definitions_col_names, index_col=0, dtype=self.definitions_dtypes)

		# Calculate synthetic features
		'''
		Convert medino from text to int
		Add classes from the classes file and calculate 4 class levels
	...
	def get_profiles(self):
		...
		# Iterate over encounters, end each encounter to self.build_enc_profiles
		for n, enc in zip(range(0, length), self.raw_profile_data.groupby(level='enc', sort=False)):
			...
			for profile in enc_profiles.groupby(level='profile', sort=False):
				...
				targets_to_append_list, pre_seq_to_append_list, post_seq_to_append_list, active_profile_to_append_list, class_1_to_append_list, class_2_to_append_list, class_3_to_append_list, class_4_to_append_list, depa_to_append_list = self.make_profile_lists(profile)
				...
				active_profiles_dict[enc[0]].extend(active_profile_to_append_list)
		...
		logging.info('Done!')
		return targets_dict, profiles_dict, pre_seq_dict, post_seq_dict, active_profiles_dict, active_classes_dict, depa_dict, enc_list
	...
	def make_profile_lists(self, profile):
		...
		depa_to_append_list = []
		profile_to_append = profile[1]['medinb'].tolist()
		for target_med in profile[1].itertuples():
			# make a list with all medications in profile
			if target_med.active == 0:
				continue
			mask = profile[1].index.get_level_values(2)==target_med.Index[2]
			target = profile[1][mask]['medinb'].astype(str).values[0]
			target_index = len(profile_to_append) - 1 - profile_to_append[::-1].index(target)
			pre_seq = profile_to_append[:target_index]
			post_seq = profile_to_append[target_index+1:]
...
if __name__ == '__main__':
	...
	parser.add_argument('--sourcefile', metavar='Type_String', type=str, nargs="?", default='data/20050101-20180101pet.csv', help='Source file load. Defaults to "data/20050101-20180101pet.csv".')
	parser.add_argument('--definitionsfile', metavar='Type_String', type=str, nargs="?", default='data/definitions.csv', help='Source file load. Defaults to "data/definitions.csv".')
